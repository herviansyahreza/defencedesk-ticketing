...
export default function TicketList() {
    ...
    useEffect(() => {
        axios.get('http://172.16.1.251:5001/solusi_populer')
            .then(response => {
    ...
    const handleSearch = async () => {
        try {
            const response = await axios.post('http://172.16.1.251:5001/search_tiket', { search: searchTerm });
            setTiket(response.data);
        } catch (error) {
    ...
    //     try {
    //         const response = await axios.delete(`http://172.16.1.251:5001/remove_tiket/${id}`);
